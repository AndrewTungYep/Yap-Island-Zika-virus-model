...
breed [relays relay]
relays-own [Bhr Bmr popspreadr distmr disthr Ahr Chr Amr mosdeathr mospopr mosspreadr]
...
to set-perameters
  set Bm random-float 0.5
  set Bh random-float 0.5
  set mospop (random 96) + 12
  set mosspread (random-float 8) + 2
  set popspread (random 16) + 5
  set Ah (random-float 0.095) + 0.132
  set Ch (random-float 0.734) + 0.174
  set distm (random-float 0.6) + 0.26
  set disth (random-float 0.5) + 0.5
  set Am (random-float 0.167) + 0.083
  set mosdeath (random-float 0.251) + 0.09
end

to extract-perameters
  ask relays [set Bm Bmr
    set Bh Bhr
    set mospop mospopr
    set mosspread mosspreadr
    set popspread popspreadr
    set Ah Ahr
    set Ch Chr
    set distm distmr
    set disth disthr
    set Am Amr
    set mosdeath mosdeathr]
end
...
to create-nodes
  foreach n-values 17 [?] [let xc item ? [73 52 50 31 36 44 79 121 108 99 122 146 119 134 128 9 8]
    ...
    ask patch xc yc [sprout 1
      ask turtles-here [set breed settlements]
      set Sh Sh + 5]]
...
to mospawn
  ...
  ask patches with [member? landtype [1 2 4 6 7 8 9 10 11 14]] [set Sm Sm + int(mos / (x + mosspread * y + (mosspread ^ 2) * z))
  ...
  while [mosx > 0] [ ask one-of patches with [member? landtype [1 2 4 6 7 8 9 10 11 14]] [set Sm Sm + 1
      set mosx mosx - 1]]
  ask patches with [member? landtype [3 5 12]] [set Sm Sm + int(mos * mosspread / (x + mosspread * y + (mosspread ^ 2) * z))
  ...
  while [mosy > 0] [ ask one-of patches with [member? landtype [3 5 12]] [set Sm Sm + 1
      set mosy mosy - 1]]
  ask patches with [landtype = 13] [set Sm Sm + int(mos * (mosspread ^ 2) / (x + mosspread * y + (mosspread ^ 2) * z))
...
to hbleed
  ask patches [if (hpop > 0) and area < 15 [set Shb binom (Sh) (pbleedh)
      ...
      let a 0
      let pat 0
      while [a = 0] [set pat patch-at-heading-and-distance (random 360) (random-exponential (2 * ln(2)))
